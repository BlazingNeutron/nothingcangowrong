...
func _on_tutorial_power_start() -> void:
	...
	self.position.x = -500
	self.position.y = -500
	tutorial_text.position.x = 650
	tutorial_text.position.y = 200
	start_overlay()
...
func _on_tutorial_sensors_start() -> void:
	...
	self.position.x = -200
	self.position.y = 200
	tutorial_text.position.x = -800
	tutorial_text.position.y = -600
	start_overlay()
...
func _on_tutorial_core_start() -> void:
	...
	self.position.x = -900
	self.position.y = 300
	tutorial_text.position.x = 350
	tutorial_text.position.y = -400
	start_overlay()
